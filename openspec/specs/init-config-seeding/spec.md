# Init Config Seeding Specification

## Purpose

This specification defines how `enpalspec init` seeds `openspec/config.yaml` with values from a `--config` seed file or an interactive context prompt at initialization time.

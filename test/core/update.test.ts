...
describe('UpdateCommand', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `enpalspec-test-${randomUUID()}`);
...
`;
      ...
      expect(updatedSkill).toContain('name: enpalspec-explore');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'enpalspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'enpalspec-explore', 'SKILL.md'),
        'old content'
      );
      ...
      const nonCoreSkillNames = [
        'enpalspec-new-change',
        'enpalspec-continue-change',
        'enpalspec-ff-change',
        'enpalspec-sync-specs',
        'enpalspec-bulk-archive-change',
      ];
  ...
  describe('command updates', () => {
    it('should update opsx commands for configured Claude tool', async () => {
      ...
      // Check enpalspec command files were created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'enpalspec');
    ...
    it('should update core profile opsx commands when tool is configured', async () => {
      ...
      const coreCommandIds = ['explore', 'apply', 'archive', 'propose', 'verify'];
      const commandsDir = path.join(testDir, '.claude', 'commands', 'enpalspec');
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'enpalspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'enpalspec-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'enpalspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'enpalspec-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: enpalspec-explore');
      ...
      expect(cursorSkill).toContain('name: enpalspec-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'enpalspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'enpalspec-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (TOML) - Qwen uses flat path structure: enpalspec-<id>.toml
      const qwenCmd = path.join(
        testDir,
        '.qwen',
        'commands',
        'enpalspec-explore.toml'
      );
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'enpalspec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'enpalspec-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'enpalspec-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'enpalspec-apply-change', 'SKILL.md'),
        'old'
      );
...
`
    ...
    it('should update all tools when force is used with mixed versions', async () => {
      ...
      const { version } = await import('../../package.json');
      const claudeSkillDir = path.join(testDir, '.claude', 'skills', 'enpalspec-explore');
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'enpalspec-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'enpalspec-explore', 'SKILL.md');
...
`;
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'enpalspec-explore', 'SKILL.md');
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'enpalspec-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'enpalspec-explore', 'SKILL.md');
    ...
    it('should not upgrade legacy tools already configured', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'enpalspec-explore'), { recursive: true });
      await fs.writeFile(
        path.join(skillsDir, 'enpalspec-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'enpalspec-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'enpalspec-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'enpalspec-new-change', 'SKILL.md');
    ...
    it('should create commands when upgrading legacy tools', async () => {
      ...
      // New enpalspec commands should be created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'enpalspec');
      const exploreCmd = path.join(commandsDir, 'explore.md');
      const exists = await FileSystemUtils.fileExists(exploreCmd);
      expect(exists).toBe(true);
    ...
    it('should not inject non-profile workflows when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'enpalspec-propose', 'SKILL.md')
      )).toBe(false);
  ...
  describe('profile-aware updates', () => {
    it('should generate only profile workflows when custom profile is set', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'enpalspec-explore'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'enpalspec-explore', 'SKILL.md'), 'old');
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'enpalspec-new-change', 'SKILL.md')
      )).toBe(true);
...
`
    ...
    it('should remove workflows outside profile during update sync', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'enpalspec-new-change'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'enpalspec-new-change', 'SKILL.md'), 'old');
      const extraCommandFile = path.join(testDir, '.claude', 'commands', 'enpalspec', 'new.md');
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'enpalspec-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'enpalspec-explore', 'SKILL.md'), 'old');
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'enpalspec-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'enpalspec-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'enpalspec-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'enpalspec-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'enpalspec-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'enpalspec-apply-change', 'SKILL.md'), 'content');
